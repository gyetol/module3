...
public class QnADaoImpl implements QnADao {
    ...
    public void insert(QnAVO qna) throws QnAException {
        String sql = "INSERT INTO qnas VALUES(DEFAULT, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, NULL, NULL);";
        template.update(sql, qna.getType().getId(), qna.getUser().getId(),
                qna.getManager().getId(), qna.getTitle(), qna.getContent());
    }

    @Override
    public void update(QnAVO qna) throws QnAException {
        String sql = "UPDATE qnas SET qna_answer_content = ? AND qna_answer_date = ? WHERE qna_id LIKE ?";
        template.update(sql, qna.getAnswerContent(), qna.getAnswerDate(), qna.getId());
    }
    ...
    public List<QnAVO> selectAll(int page, int pageSize) throws QnAException {
        ...
        String sql = "SELECT * FROM qnas ORDER BY qna_question_date ASC LIMIT ?, ?;";
    ...
    public List<QnAVO> selectAll(int page, int pageSize, String qna_type) throws QnAException {
        List<QnAVO> list;
        int startPoint = (page-1)*pageSize;
        String sql = "SELECT * FROM qnas WHERE qna_type LIKE ? ORDER BY qna_question_date ASC LIMIT ?, ?;";
    ...
    public QnAVO selectById(int qna_id) throws QnAException {
        List<QnAVO> list;
        String sql = "SELECT * FROM qnas WHERE qna_id = ?;";
    ...
    public int getTotalRecord(String qna_type) throws QnAException {
        List<Integer> list = template.query("select count(*) as num from qna_types WHERE qna_type LIKE " +qna_type+";", new RowMapper<Integer>() {
            @Override
            public Integer mapRow(ResultSet rs, int rowNum) throws SQLException {
                return rs.getInt("num");
            }
